...
export default function SingleChat({ fetchAgain, setFetchAgain }) {
  ...
  useEffect(() => {
    ...
    socket.on("typing", (user1) => user1 !== user && setIsTyping(true));
    ...
    socket.on("message recieved", (message) => {
      if (
        ...
        notification.forEach((notif) => {
          if (message.chat.isGroupChat && notif.chat.isGroupChat) {
            if (message.chat.chatName === notif.chat.chatName) {
              flag = false;
            }
          } else if (!message.chat.isGroupChat && !notif.chat.isGroupChat) {
            if (
              getSender(user, message.chat.users) ===
